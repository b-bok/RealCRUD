...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * 
			 * 1 kbyte == 1024byte 1 mbyte == 1024kbyte => 1024*1024byte 10 mbyte ==
			 * 10*1024*1024 byte
			 * 
			 */
			 ...
			 * 
			 * 
			 * 위의 구문만으로 넘어온 첨부파일은 내가 지정한 폴더에 업로드가 됨 => 즉, 문제가 있든 없든 간에 무조건 서버에 업로드 되버림... =>
			 * 즉, 혹시 DB에 INSERT할때 문제가 생기면 해당 업로드 된 파일 지우는게 좋음! => 메모리 관리!
			 * 
			 * 
			 * 첨부한 파일을 업로드 할 때 원본명 그대로 업로드 하지 않는 게 일반적임! - 중복된 이름 파일이 있을 수 도 있고,
			 * 한글/특수문자/띄어쓰기가 포함된 파일일 수 있음!
			 * 
			 * 
			 * => 기본적인 수정명 작업을 해주는 객체 => cos에서 제공하는 DefaultFileRenamePolicy객체 => 내부적으로
			 * rename() 메소드가 실행되면서 파일명 수정진행됨! => 기존에 동일한 파일명이 있을 경우 카운팅 된 숫자 붙여줌! ex)
			 * aaa.jpg, aaa1.jpg
			 * 
			 * 단, 내 입맛대로 수정명작업 가능!!
			 * 
